...
@variable(model, b[i in M, j in N], Bin)

# Define the Objective Function
@objective(model, Min, sum(c[k] * a[i, j, k] for i in M, j in N, k in S) + 1000000 * sum(b[i, j] for i in M, j in N))
...
for (req_x, req_y) in req
    ...
    @constraint(model, 1 - sum(a[i, j, k] for (i, j, k) in constraint_vars) <= 1000000 * b[req_x, req_y])
end
...
p = plot(hms..., layout=(1, 5), size=(1500, 300), show=true, 
    suptitle="Total cost: $(objective_value(model)), filename: $filename, formulation 2",
    top_margin=2Plots.mm, bottom_margin=2Plots.mm)
savefig(p, "tmp.png")
